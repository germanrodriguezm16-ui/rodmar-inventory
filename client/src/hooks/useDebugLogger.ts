...
const MAX_LOGS = 100;
const LOG_PATTERNS = [/^🔔/, /^📱/, /^📨/, /^📤/, /^💾/, /^🔍/, /^🔎/, /^📋/, /^✅/, /^⚠️/, /^❌/, /^📬/, /^⏳/, /RodMar PWA:/, /Service Worker:/];
...
class DebugLogger {
  ...
  private shouldLog(message: string): boolean {
    return LOG_PATTERNS.some(p => p.test(message));
  }
  ...
  start() {
    ...
    console.log = (...args: any[]) => {
      ...
      if (this.shouldLog(msg)) this.addLog('info', msg, args.slice(1));
    };
  }
...
export function useDebugLogger() {
  ...
  return { logs, isActive, startLogging: start, stopLogging: stop, clearLogs: clear };
}
