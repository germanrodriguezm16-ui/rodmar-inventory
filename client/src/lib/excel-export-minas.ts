...
export function previewMinaTripHistory(trips: ViajeWithDetails[]): any[] {
  ...
  const exportData = previewTrips.map(trip => ({
    ...
    'COMPRA': trip.totalCompra ? formatCurrency(trip.totalCompra) : '-',
    'ESTADO': trip.estado || '-'
  }));

  // Calcular totales
  const totales = exportData.reduce((acc, trip) => {
    const totalCompra = parseFloat(trip['COMPRA'].replace(/[\$,.]/g, '')) || 0;
  ...
  }, { count: 0, totalCompra: 0, totalPeso: 0 });
  ...
  const filaTotales = {
    ...
    'CUT': '',
    'COMPRA': '$' + Math.round(totales.totalCompra).toString().replace(/\B(?=(\d{3})+(?!\d))/g, '.'),
    'ESTADO': 'TOTALES'
  };
...
export function exportMinaTripHistory(trips: ViajeWithDetails[], minaNombre: string) {
  try {
    // Preparar datos para exportación con formato específico del historial de minas
    ...
    for (let row = 1; row <= exportData.length; row++) {
      ...
      for (let col = range.s.c; col <= range.e.c; col++) {
        ...
        if (ws[cellRef]) {
          ...
          const headers = ['ID', 'FECHA DE CARGUE', 'FECHA DE DESCARGUE', 'CONDUCTOR', 'TIPO DE CARRO', 'PLACA', 'PESO (Ton)', 'CUT', 'COMPRA', 'ESTADO'];
          const header = headers[colIndex];
          if (header && ['PESO (Ton)', 'CUT', 'COMPRA'].includes(header)) {
    ...
    for (let col = range.s.c; col <= range.e.c; col++) {
      ...
      if (ws[cellRef]) {
        ...
        const headers = ['ID', 'FECHA DE CARGUE', 'FECHA DE DESCARGUE', 'CONDUCTOR', 'TIPO DE CARRO', 'PLACA', 'PESO (Ton)', 'CUT', 'COMPRA', 'ESTADO'];
        const header = headers[colIndex];
        if (header && ['PESO (Ton)', 'COMPRA'].includes(header)) {
          ws[cellRef].s.alignment = { ...ws[cellRef].s.alignment, horizontal: "right" };
        }
      }
    }
    ...
    wb.Props = {
      Title: `Historial de Viajes - ${minaNombre}`,
      Subject: `Exportación de historial de viajes para mina ${minaNombre}`,
      Author: "Sistema RodMar",
      CreatedDate: new Date(),
      Comments: `Generado el ${new Date().toLocaleDateString('es-CO')} con ${exportData.length} viajes de la mina ${minaNombre}`
    };

    // Generar archivo con nombre descriptivo
    const fecha = new Date().toLocaleDateString('es-CO').replace(/\//g, '-');
    const fileName = `RodMar_${minaNombre.replace(/[^a-zA-Z0-9]/g, '_')}_Historial_${fecha}_${exportData.length}viajes.xlsx`;
    XLSX.writeFile(wb, fileName);
    
    return true;
  } catch (error) {
    console.error('Error exportando Excel de historial de minas:', error);
