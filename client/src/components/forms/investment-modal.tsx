...
const inversionSchema = z.object({
  ...
  valor: z.string().min(1, "Valor es requerido").refine((val) => !isNaN(Number(val)), "Debe ser un número válido"),
...
interface InvestmentModalProps {
  isOpen: boolean;
  onClose: () => void;
  subpestana: string; // "santa-rosa" o "cimitarra"
}

const origenOptions = [
  { value: "rodmar", label: "RodMar" },
  { value: "banco", label: "Banco" },
  { value: "lcdm", label: "LCDM" },
...
export function InvestmentModal({ isOpen, onClose, subpestana }: InvestmentModalProps) {
  const { toast } = useToast();
  const queryClient = useQueryClient();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [selectedOrigen, setSelectedOrigen] = useState<string>("");
  const [selectedDestino, setSelectedDestino] = useState<string>("postobon");
  ...
  const createInversionMutation = useMutation({
    mutationFn: async (data: InversionFormData) => {
      return await apiRequest("POST", "/api/inversiones", {
        ...data,
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: [`/api/inversiones/cuenta/${subpestana}`] });
      queryClient.invalidateQueries({ queryKey: [`/api/inversiones/cuenta/${destino === 'rodmar' ? destinoDetalle : destino}`] });
      form.reset();
      onClose();
    },
    onError: (error: any) => {
      toast({
        title: "Error",
        description: error.message || "No se pudo crear la inversión",
        variant: "destructive",
      });
    },
  });

  const onSubmit = async (data: InversionFormData) => {
    setIsSubmitting(true);
    try {
      await createInversionMutation.mutateAsync(data);
    } finally {
      setIsSubmitting(false);
    }
  };

  const formatSubpestanaTitle = (subpestana: string) => {
    return subpestana === "santa-rosa" ? "Santa Rosa" : "Cimitarra";
  };
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <DialogTitle className="text-sm sm:text-base truncate">
                Nueva Inversión - {formatSubpestanaTitle(subpestana)}
              </DialogTitle>
            </div>
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            {form.formState.errors.origen && (
              <p className="text-xs text-red-500">{form.formState.errors.origen.message}</p>
            )}
          </div>
          ...
          {/* Detalle del Origen para otros casos */}
          {selectedOrigen && !["rodmar", "postobon"].includes(selectedOrigen) && (
            <div className="space-y-1">
              <Label htmlFor="origenDetalle" className="text-xs font-medium">Detalle (opcional)</Label>
              <Input
                {...form.register("origenDetalle")}
                placeholder="Especificar cuenta"
          ...
          {selectedDestino === "postobon" && (
            <div className="space-y-1">
              ...
              <Select
                value={form.watch("destinoDetalle") || subpestana}
                onValueChange={(value) => form.setValue("destinoDetalle", value)}
              >
                <SelectTrigger className="h-8 text-sm">
                  <SelectValue placeholder="Seleccionar cuenta" />
          ...
          {/* Detalle del Destino para otros casos */}
          {selectedDestino && !["rodmar", "postobon"].includes(selectedDestino) && (
            <div className="space-y-1">
              <Label htmlFor="destinoDetalle" className="text-xs font-medium">Detalle (opcional)</Label>
              <Input
                {...form.register("destinoDetalle")}
                placeholder="Especificar cuenta"
                className="h-8 text-sm"
              />
            </div>
          ...
          <div className="space-y-1">
            ...
            {form.formState.errors.concepto && (
              <p className="text-xs text-red-500">{form.formState.errors.concepto.message}</p>
            )}
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 pt-3 modal-buttons-container">
            ...
            <Button 
              type="submit" 
              disabled={isSubmitting || createInversionMutation.isPending}
