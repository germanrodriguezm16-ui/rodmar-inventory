...
export const requireAuth: RequestHandler = async (req, res, next) => {
  try {
    logger.debug("🔐 [AUTH] Verificando autenticación para:", req.path);
    
    // Obtener token del header Authorization
    const authHeader = req.headers.authorization;
    if (!authHeader || !authHeader.startsWith("Bearer ")) {
      logger.debug("❌ [AUTH] No hay token en el header Authorization");
      return res.status(401).json({ error: "No autenticado" });
    }

    const token = authHeader.substring(7); // Remover "Bearer "
    logger.debug("🔑 [AUTH] Token recibido:", token.substring(0, 20) + "...");

    // Verificar token
    const tokenData = verifyToken(token);
    if (!tokenData) {
      logger.debug("❌ [AUTH] Token inválido o expirado");
      return res.status(401).json({ error: "Token inválido o expirado" });
    }

    const userId = tokenData.userId;
    logger.debug("✅ [AUTH] Token válido para usuario:", userId);
