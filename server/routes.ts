...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.patch("/api/transacciones/:id/hide-comprador", async (req, res) => {
    try {
      const userId = req.user?.id || "main_user";
      const transactionId = parseInt(req.params.id);
  ...
  app.patch("/api/transacciones/:id/hide-mina", async (req, res) => {
    try {
      const userId = req.user?.id || "main_user";
      const transactionId = parseInt(req.params.id);
  ...
  app.patch("/api/transacciones/:id/hide-volquetero", async (req, res) => {
    try {
      const userId = req.user?.id || "main_user";
      const transactionId = parseInt(req.params.id);
  ...
  app.patch("/api/transacciones/:id/hide-general", async (req, res) => {
    try {
      const userId = req.user?.id || "main_user";
      const transactionId = parseInt(req.params.id);
      ...
      if (success) {
        res.json({
          success: true,
          message: "Transacción ocultada en módulo general",
        });
      } else {
        res.status(404).json({ error: "Transacción no encontrada" });
      }
